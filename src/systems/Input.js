...
export const InputSystem = {
  ...
  handleGamepad: (gp) => {
    ...
    // --- HOTBAR SLOT NAVIGATION (ALWAYS ACTIVE) ---
    if (isPressed(gp, 14) && !InputSystem.navLock) { // D-Left
    ...
    if (isPressed(gp, 15) && !InputSystem.navLock) { // D-Right
        let s = parseInt(State.hotbar.activeSlot) || 1;
    ...
    // --- CONTEXT SENSITIVE LOGIC (Terminal Open vs Closed) ---
    if (State.player.isTerminalOpen) {
        // Mode A: Terminal Navigation & Binding
        if (isPressed(gp, 12) && !InputSystem.navLock) { if(TerminalSystem.cycleControllerCommand) TerminalSystem.cycleControllerCommand(-1); InputSystem.navLock = true; } // D-Up
        if (isPressed(gp, 13) && !InputSystem.navLock) { if(TerminalSystem.cycleControllerCommand) TerminalSystem.cycleControllerCommand(1); InputSystem.navLock = true; } // D-Down
        if (isPressed(gp, 0) && !InputSystem.navLock) { // A -> EXECUTE
            const val = document.getElementById("term-input").value.trim();
            if (val) { TerminalSystem.print("> " + val, "#888", "cmd"); TerminalSystem.execute(val); }
            InputSystem.navLock = true;
        }
        
        // ---> FIX IS HERE: X/Square only binds when terminal is open <---
        ...
        if (isPressed(gp, 0) && !InputSystem.btnLock) { // A -> EXECUTE ACTIVE SLOT
            let s = parseInt(State.hotbar.activeSlot) || 1;
            if (State.hotbar[s]) TerminalSystem.execute(State.hotbar[s]);
            InputSystem.btnLock = true;
        }
        
        // ---> FIX IS HERE: X/Square only opens Objective when terminal is closed <---
        if (isPressed(gp, 2) && !InputSystem.btnLock) { 
    ...
    // Reset Locks safely
    if (!isPressed(gp, 3) && !isPressed(gp, 2) && !isPressed(gp, 1) && !isPressed(gp, 9) && !isPressed(gp, 0)) InputSystem.btnLock = false;
